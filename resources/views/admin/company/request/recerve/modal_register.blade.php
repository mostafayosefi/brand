...
<button type="button" class="btn btn-primary  "  data-toggle="modal" data-target="#active" ><i data-feather="check-square"></i>
    ثبت نهایی اطلاعات شرکت  </button>
...
<div class="modal-header">
  <h5 class="modal-title" id="exampleModalLabel"> ثبت نهایی اطلاعت شرکت </h5>
...
<div class="modal-body">
 ...
 @csrf
   ...
   <div class="form-group">
       ...
       placeholder=" شماره Number      " name="number" value="{{ old('number') }}" required>
  </div>
   ...
   <div class="form-group">
       ...
       placeholder=" کدانتقال" name="authcode" value="{{ old('authcode') }}" required>
  </div>
...
<div class="form-group ">
    <label for="date">   تاریخ اکسپایر  </label>
      ...
      type="text" {{ old('dateconfermation') }}   required />
   </div>
...
<textarea name="office" class="form-control" rows="8"  required="required" > {{old('office') }}</textarea>
</div>

<input type="hidden" name="company_request_id" value="{{$order->id}}">
<input type="hidden" name="user_id" value="{{$order->user->id}}">

<button type="submit" class="btn btn-primary btn-block"> ثبت نهایی اطلاعت شرکت <i data-feather="check-square"></i></button>
                             </div>
